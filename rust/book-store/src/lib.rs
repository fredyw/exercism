...
pub fn lowest_price(books: &[u32]) -> u32 {
    ...
    fn lowest_price<'a>(book_counts: [i32; 5], memo: &mut HashMap<[i32; 5], u32>) -> Option<u32> {
        ...
        for i in 0..5 {
            let mut new_book_counts = book_counts.clone();
            new_book_counts[i] -= 1;
            if let Some(p) = lowest_price(new_book_counts, memo) {
        ...
        memo.insert(book_counts, answer);
        Some(answer)
    }
